...
def performance_function_1(agent: twipr_ecilc.TWIPR_2D_ECILC_Agent):
    if agent.active:
        ...
        # if len(agent.ilc.trials) > 1:
        #     agent.performance = agent.ilc.trials[-2].e_norm / agent.ilc.trials[-1].e_norm
        #     if agent.performance < 1:
        #         agent.performance = 1e-14
...
config_1 = twipr_ilc.config_4 | {'entry_trial': 0, 'performance_function': performance_function_1,
...
agents_baseline = [agent1_baseline, agent2_baseline]
...
agents_cilc = [agent1_cilc, agent2_cilc]
...
enorm_cilc = ilc.minimum_error_norm(agent1_cilc.ilc.e_norm, agent2_cilc.ilc.e_norm)
...
enorm_ecilc = core.simulation.Timeseries(
    ilc.minimum_error_norm(agent1.ilc.e_norm, agent2.ilc.e_norm))
