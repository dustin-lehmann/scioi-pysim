...
class Robot:
    ...
    def __init__(self, id, length, width, height, environment=None):
        self.id = id
        self.length = length
        self.width = width
        self.height = height
...
class Switch:
    ...
    def check(self, robots: dict[str, Robot]):
...
class Environment:
    ...
    def toJson(self, file=None):
        ...
        for name, switch in self.switches.items():
            switch_dict = {
                'name': name,
                'position': self.getCellPosition(switch.cell),
                'circuit': switch.circuit_id
            }
            json_dict['switches'][name] = switch_dict

        for name, door in self.doors.items():
            door_dict = {
                'name': name,
                'position': list(door.center),
                'psi': door.psi,
                'length': door.size_x,
                'circuit': door.circuit_id,
                'visible': door.visible
            }
            json_dict['doors'][name] = door_dict

        for name, goal in self.goals.items():
            goal_dict = {
                'name': name,
                'position': self.getCellPosition(goal.cell),
                'visible': goal.visible,
                'size': self.tile_size,
            }
            json_dict['goals'][name] = goal_dict
